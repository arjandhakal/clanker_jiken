...
following GPT-5.5/5.6 combinations are supported by pi 0.75:

| Provider | Model | Supported `thinking` values |
|---|---|---|
| `openai-codex` | `gpt-5.5` | `off`, `minimal` (maps to low), `low`, `medium`, `high`, `xhigh` |
| `openai-codex` | `gpt-5.6-luna` | `off`, `minimal` (maps to low), `low`, `medium`, `high`, `xhigh`, `max` |
| `openai-codex` | `gpt-5.6-sol` | `off`, `minimal` (maps to low), `low`, `medium`, `high`, `xhigh`, `max` |
| `openai-codex` | `gpt-5.6-terra` | `off`, `minimal` (maps to low), `low`, `medium`, `high`, `xhigh`, `max` |
...
The extension passes the selected value through to `pi --thinking`; unsupported combinations
are clamped by pi. Omit `thinking` to use pi's default.
