...
so a gate either exists as an extension or it does not exist at all. This local fork judges
`bash`, `write`, and `edit` tool calls semantically and asks you to choose when a call would
...
everything else         → Jev: allow · ask user when it would block
```
...
dangerous shape. If Jev clearly rejects the call, cannot decide, has no key, or the uncertain
band is configured as `deny`, this fork opens a user decision prompt instead of immediately
blocking.
...
Each decision leaves a record in the transcript. `display: "compact"` (the default) folds an
approval into one line and a block into two (the rationale stays, because a block is something
to act on). `display: "full"` shows the heading, the engine line, the command, the reasons, and
the rationale. The expanded view shows every detail in either mode.
...
nothing vouches for asks you whether to allow once, allow always, or block.
...
Without a key the gate does not disable itself and does not allow everything: it asks before
calls it cannot judge and says it is not connected to Jev. The footer shows `🛡 jev (<scope>)`
while the semantic layer is active and `🛡 jev no key (<scope>)` when it is not.
