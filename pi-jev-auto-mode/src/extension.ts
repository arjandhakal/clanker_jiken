...
async function askUserToResolveBlock(
  ...
  const { call, reasons, rationale, evidence, originalSource } = input;
  ...
  if (!ctx.hasUI) {
    return blocked(
      deps,
      {
        ...
        status: "blocked",
        source: "no-ui",
        rationale: `${rationale} No UI is available to choose whether to allow the call.`,
        evidence,
      },
  ...
  writeRecord(deps, {
    ...
    status: "cancelled",
    source: "user",
    rationale: `The user blocked the call. Original decision: ${rationale}`,
    evidence,
  });
  return { block: true, reason: `Blocked by the user at the Jev auto mode decision prompt. ${rationale}` };
}
...
export async function evaluateToolCall(
  ...
  // inventing a verdict, while still recording the unavailable semantic layer.
  if (deps.engine.id === MANUAL_ENGINE_ID) {
    const rationale = NO_ENGINE_MESSAGE;
    return askUserToResolveBlock(ctx, state, deps, {
      call,
      reasons,
      status: "blocked",
      source: "unavailable",
  ...
  switch (verdict.verdict) {
    ...
    case "uncertain": {
      ...
      const rationale =
        state.settings.uncertain === "deny"
          ? `No condition decided the call, and the uncertain band is resolved to a block. ${verdict.rationale}`
          : verdict.rationale;
      return askUserToResolveBlock(ctx, state, deps, {
        call,
        reasons,
        status: "blocked",
        source: state.settings.uncertain === "deny" ? "uncertain" : "user",
        originalSource: state.settings.uncertain === "deny" ? "uncertain" : "user",
        rationale,
        evidence,
      });
    }
