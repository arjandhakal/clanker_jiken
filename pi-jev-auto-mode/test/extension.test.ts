...
describe("no Jev connection", () => {
  ...
  it("asks the user when Jev is unavailable", async () => {
    const ui = createUi({ answer: "Allow once" });
    const { deps, records, inputs } = createDeps({ engine: manual });
    const result = await evaluateToolCall(bash("mkdir -p notes"), createContext({ ui: ui.ui }), stateWith(), deps);
    ...
    assert.match(records[0]?.rationale ?? "", /Not connected to Jev/);
...
describe("semantic verdicts", () => {
  ...
  it("lets the user block a rejected command", async () => {
  ...
  it("asks when no decision is available and honours allow once", async () => {
    ...
    assert.equal(records[0]?.source, "user");
    assert.equal(records[0]?.status, "confirmed");
    assert.match(records[0]?.rationale ?? "", /timeout/);
    assert.equal(ui.selections.length, 1);
  });
  ...
  it("asks when the engine throws", async () => {
    const engine: DecisionEngine = {
      id: "broken",
      judge: async () => {
...
describe("the uncertain band", () => {
  ...
  it("asks on an unclear call when configured to deny", async () => {
    const ui = createUi({ answer: "Allow once" });
    const { deps, records } = createDeps({ verdict: uncertain });
    const result = await evaluateToolCall(
      bash("git clean -fd"),
...
describe("file tools", () => {
  ...
  it("escalates a write that escapes the working directory and lets the user block it", async () => {
